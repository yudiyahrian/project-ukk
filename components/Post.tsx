...
import SavedPostButton from "./SavePostButton";
        ...
        <div className="rounded-md bg-white shadow">
          <div className="px-6 py-4 flex justify-between">
            <div className="w-0 flex-1">
              ...
              <a href={`/post/${post.id}`}>
                <h1 className="text-lg font-semibold pt-1 leading-6 text-gray-900">
                  {post.title}
                </h1>
              </a>

              {post.description && (
                <p className="text-zinc-500 text-sm pt-2">{post.description}</p>
              )}

              {post.photos.length !== 0 && (
                <div
                  className="relative text-sm h-fit w-full pt-2"
                  ref={postRef}
                >
          ...
          <div className="flex gap-2 bg-gray-50 z-20 text-sm p-4 sm:px-6">
            ...
            <SavedPostButton postId={post.id} initialSaved={currentSaved} />
          </div>
        </div>
      </>
    )
